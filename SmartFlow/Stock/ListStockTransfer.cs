...
{
    public partial class ListStockTransfer : Form
    {
        private static string connectionString = ConfigurationManager.ConnectionStrings["ConnectionString"].ConnectionString;
        public ListStockTransfer()
        {
            InitializeComponent();
        }

        private void ListStockTransfer_Load(object sender, EventArgs e)
        {
        ...
        private void searchbtn_Click(object sender, EventArgs e)
        {
            ...
            string query = @"SELECT CASE WHEN ROW_NUMBER() OVER (PARTITION BY StockCustomizedTable.InvoiceNo ORDER BY StockTable.ProductID) = 1
                ...
                StockTable.TransferFromWarehouseID,
                TransferFromWarehouse.Name AS [Transfer From Warehouse],
                StockTable.TransferToWarehouseID,
                TransferToWarehouse.Name AS [Transfer To Warehouse],
                ...
                JOIN WarehouseTable AS TransferFromWarehouse ON TransferFromWarehouse.WarehouseID = StockTable.TransferFromWarehouseID
                JOIN WarehouseTable AS TransferToWarehouse ON TransferToWarehouse.WarehouseID = StockTable.TransferToWarehouseID
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                {
                    this.Close();
                    DetailStockTransfer detailStockTransfer = new DetailStockTransfer(dataInvoice, startDate, endDate);
                    detailStockTransfer.MdiParent = Application.OpenForms["Dashboard"];
                    CommonFunction.DisposeOnClose(detailStockTransfer);
                    detailStockTransfer.Show();
                }
            }
        }
    }
