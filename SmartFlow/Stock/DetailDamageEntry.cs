...
{
    public partial class DetailDamageEntry : Form
    {
        private DataTable _invoiceData;
        private DateTime _startdate;
        private DateTime _enddate;
        public DetailDamageEntry(DataTable invoiceData, DateTime startdate, DateTime enddate)
        {
            InitializeComponent();
        ...
        private void DetailDamageEntry_Load(object sender, EventArgs e)
        {
            dgvlistdamage.DataSource = _invoiceData;
            dgvlistdamage.Columns["Description"].DefaultCellStyle.WrapMode = DataGridViewTriState.True;
            dgvlistdamage.Columns["Transfer From Warehouse"].DefaultCellStyle.WrapMode = DataGridViewTriState.True;
            dgvlistdamage.Columns["Transfer To Warehouse"].DefaultCellStyle.WrapMode = DataGridViewTriState.True;
            dgvlistdamage.Columns["Item Details"].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
            dgvlistdamage.Columns["ProductID"].Visible = false;
            dgvlistdamage.Columns["TransferFromWarehouseID"].Visible = false;
            dgvlistdamage.Columns["TransferToWarehouseID"].Visible = false;
            dgvlistdamage.Columns["Invoice"].Visible = false;
            dgvlistdamage.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCells;
            fromdatevaluelbl.Text = _startdate.ToString();
            todatevaluelbl.Text = _enddate.ToString();
        }

        private async void dgvlistdamage_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            // Ensure that a valid row is double-clicked
            if (e.RowIndex >= 0)
            {
                // Get the InvoiceNo from the selected row
                var invoiceNo = dgvlistdamage.CurrentRow.Cells["Invoice"].Value.ToString();
                ...
                if (dataInvoice.Rows.Count > 0)
                {
                    ...
                    {
                        DamageEntry damageEntry = new DamageEntry(dataInvoice, dtInvoiceDetails);
                        damageEntry.MdiParent = Application.OpenForms["Dashboard"];
        ...
        private async void dgvlistdamage_KeyDown(object sender, KeyEventArgs e)
        {
            // Check if the Enter key is pressed
            if (e.KeyCode == Keys.Enter)
            {
                // Ensure that a valid row is selected
                if (dgvlistdamage.CurrentRow != null)
                {
                    // Get the InvoiceNo from the selected row
                    var invoiceNo = dgvlistdamage.CurrentRow.Cells["Invoice"].Value.ToString();
