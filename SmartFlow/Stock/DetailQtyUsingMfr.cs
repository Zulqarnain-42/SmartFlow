...
{
    public partial class DetailQtyUsingMfr : Form
    {
        private DataTable _invoiceData;
        private DateTime _startdate;
        private DateTime _enddate;
        public DetailQtyUsingMfr(DataTable invoiceData, DateTime startdate, DateTime enddate)
        {
            InitializeComponent();
        ...
        private void DetailQtyUsingMfr_Load(object sender, EventArgs e)
        {
            dgvlistusingmfr.DataSource = _invoiceData;
            dgvlistusingmfr.Columns["Description"].DefaultCellStyle.WrapMode = DataGridViewTriState.True;
            dgvlistusingmfr.Columns["Warehouse"].DefaultCellStyle.WrapMode = DataGridViewTriState.True;
            dgvlistusingmfr.Columns["Item Details"].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
            dgvlistusingmfr.Columns["ProductID"].Visible = false;
            dgvlistusingmfr.Columns["Warehouseid"].Visible = false;
            dgvlistusingmfr.Columns["Invoice"].Visible = false;
            dgvlistusingmfr.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCells;
            fromdatevaluelbl.Text = _startdate.ToString();
            todatevaluelbl.Text = _enddate.ToString();
        }

        private async void dgvlistusingmfr_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            // Ensure that a valid row is double-clicked
            if (e.RowIndex >= 0)
            {
                // Get the InvoiceNo from the selected row
                var invoiceNo = dgvlistusingmfr.CurrentRow.Cells["Invoice"].Value.ToString();
                ...
                if (dataInvoice.Rows.Count > 0)
                {
                    ...
                    {
                        QtyUsingMfr qtyUsingMfr = new QtyUsingMfr(dataInvoice, dtInvoiceDetails);
                        qtyUsingMfr.MdiParent = Application.OpenForms["Dashboard"];
        ...
        private async void dgvlistusingmfr_KeyDown(object sender, KeyEventArgs e)
        {
            // Check if the Enter key is pressed
            if (e.KeyCode == Keys.Enter)
            {
                // Ensure that a valid row is selected
                if (dgvlistusingmfr.CurrentRow != null)
                {
                    // Get the InvoiceNo from the selected row
                    var invoiceNo = dgvlistusingmfr.CurrentRow.Cells["Invoice"].Value.ToString();
