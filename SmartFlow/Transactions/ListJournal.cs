...
{
    public partial class ListJournal : Form
    {
        ...
        private void ListJournal_Load(object sender, EventArgs e)
        {
        ...
        private void searchbtn_Click(object sender, EventArgs e)
        {
            ...
            string invoicePattern = "JR-%";

            // Use parameterized SQL query to avoid issues with direct string concatenation
            string query = @"SELECT T.TransactionId, T.InvoiceNo, T.InvoiceNo AS HIDDENINVOICE, T.InvoiceDate, T.TransactionCode, T.LongDescription, 
                    T.CurrencyId, T.CurrencyName, T.CurrencySymbol, T.ConversionRate, T.CreatedAt, T.UpdatedAt, T.CreatedDay, T.UpdatedDay,
                    T.UserId, T.AddedBy, T.CompanyId, T.VoucherInfo, TD.TransactionDetailId, TD.TransactionId, TD.AccountId, TD.AccountName,
                    TD.AccountCode, TD.IsDebit, TD.IsCredit, TD.ShortDescription, TD.DebitAmount, TD.CreditAmount, TD.InvoiceNo, TD.DebitOrCredit,
                    TD.TransactionCode
                    ...
                    AND TD.IsNewRecord = @IsNewRecord 
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                {
                    this.Close();
                    DetailJournal detailJournal = new DetailJournal(dataInvoice);
                    detailJournal.MdiParent = Application.OpenForms["Dashboard"];
                    CommonFunction.DisposeOnClose(detailJournal);
                    detailJournal.Show();
                }
            }
