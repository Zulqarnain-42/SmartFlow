...
namespace SmartFlow.Transactions
{
    public partial class MaterialReceivedFromParty : Form
    {
        ...
        public MaterialReceivedFromParty(DataTable dtinvoice,DataTable dtinvoicedetails)
        {
            InitializeComponent();
        ...
        private async Task<string> GetLastInvoiceNumber()
        {
            ...
            try
            {
                ...
                string query = "SELECT TOP 1 InvoiceNo FROM InvoiceTable WHERE InvoiceNo LIKE 'MRP-%' ORDER BY InvoiceNo DESC";
            ...
            try
            {
                    ...
                    && !string.IsNullOrEmpty(currencyconversionratelbl.Text) && currencyconversionratelbl.Text != "currencyconversionratelbl")
                {
                    ...
                    if (savebtn.Text == "UPDATE")
                    {
                        ...
                        {
                            await UpdateMaterialReceived(invoiceNo, invoiceDate);
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        if (DateTime.TryParseExact(invoicedatetxtbox.Text, format, cultureInfo, System.Globalization.DateTimeStyles.None, out invoiceDate))
                        {
                            detailadded = await AddMaterialReceived(invoiceNo, invoiceDate);
                        }
                        else
                        {
                            invoiceDate = DateTime.Parse(invoicedatetxtbox.Text);
                    ...
                    if (detailadded)
                    {
                        ...
                        MaterialReceivedReportView materialReceivedReportView = new MaterialReceivedReportView(invoiceNo);
                        materialReceivedReportView.MdiParent = Application.OpenForms["Dashboard"];
                        CommonFunction.DisposeOnClose(materialReceivedReportView);
                        materialReceivedReportView.Show();
                        this.Close();
                    }
                }
                else
        ...
        private async Task UpdateMaterialReceived(string invoiceNo, DateTime invoiceDate)
        {
        ...
        private async Task<bool> AddMaterialReceived(string invoiceNo, DateTime invoiceDate)
        {
        ...
        private async void dgvsaleproducts_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Control && e.KeyCode == Keys.D)
            {
                // Ensure the current row has a valid product ID.
                var productidCellValue = dgvsaleproducts.CurrentRow?.Cells["productid"].Value;
